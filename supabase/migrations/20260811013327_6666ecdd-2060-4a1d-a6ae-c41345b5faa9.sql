CREATE POLICY "own invoice files select" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'notas-fiscais' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "own invoice files insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'notas-fiscais' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "own invoice files update" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'notas-fiscais' AND (storage.foldername(name))[1] = auth.uid()::text)
  WITH CHECK (bucket_id = 'notas-fiscais' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "own invoice files delete" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'notas-fiscais' AND (storage.foldername(name))[1] = auth.uid()::text);